...
export default function GalleryLightbox({ photos }: { photos: GalleryPhoto[] }) {
  ...
  return <>
    {photos.map((photo, photoIndex) => <button
      ...
      onClick={(event) => { trigger.current = event.currentTarget; setIndex(photoIndex); }}
    >
      <img className="aspect-[4/3] w-full object-cover transition duration-500 group-hover:scale-105" src={photo.src} alt={photo.alt} loading="lazy" />
    ...
    {index !== null && <div className="fixed inset-0 z-[70] flex items-center justify-center bg-[#0c1718]/90 p-4 sm:p-5" role="dialog" aria-modal="true" aria-label="照片预览" onClick={close}>
      ...
      <figure className="max-h-[90vh] max-w-5xl" onClick={(event) => event.stopPropagation()}>
        <img className="max-h-[78vh] w-auto max-w-full rounded-[1.1rem] object-contain" src={photos[index].src} alt={photos[index].alt} />
